...
| 2 | `2_sdd.md` | SDD 두뇌 | `docs/{prd,architecture,adr}` | PRD/SDD 승인 |
| 3 | `3_agents.md` | 목록 제안→승인→템플릿 기반 생성 | `.claude/agents/*` | 에이전트 집합 |
| 4 | `4_skills.md` | 스킬 제안→승인→생성 + 오케스트레이터 | `.claude/skills/*` | 스킬 집합 |
| 5 | `5_constitution.md` | 헌법 + 가드레일 | `CLAUDE.md`, `.claude/{settings.json,hooks}` | — |
| 6 | `6_engine.md` | 자율 실행 엔진 | `scripts/*`, `.claude/commands/{harness,review}.md` | — |
| 7 | `7_validate.md` | 검증·테스트 | 검증 리포트 | 가동 준비 |

공통:
- 각 step은 전용 지침을 내장하며, 횡단 kb는 `kb/README.md` 로드맵의 파일만 로드(컨텍스트 절약).
- 매 step 종료 시 `kb/docs-rules.md`(result_report) 규칙대로 `docs/result_report/`에 짧은 리포트.
