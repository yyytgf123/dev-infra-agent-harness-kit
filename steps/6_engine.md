...
2. `templates/engine/phase.json.tmpl` → `scripts/phase.json` (빈 골격, `approved:false`, `mode`=step1 판정).
   - **`env_checks`를 step0 확정 스택으로 채운다** (예: `java -version` → "21") — 실행 환경이 확정 버전과
     다르면 execute.sh가 시작 거부(린트 게이트가 깨진 채 도는 것 방지). 버전 확인 명령은 `kb/tooling-matrix.md`.
   phases[]는 비워 두거나 예시만 — 실제 분할은 가동 시 `/harness`가 채운다.
3. `templates/engine/harness.md.tmpl` → `.claude/commands/harness.md` (`/harness` 슬래시 커맨드로 등록됨).
4. `templates/engine/review.md.tmpl` → `.claude/commands/review.md` (`/review` 슬래시 커맨드).
