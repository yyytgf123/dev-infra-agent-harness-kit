...
아래 나오는 `steps/..`·`kb/..`·`templates/..` 경로는 모두 이 키트 루트 기준이다.
...
#   - 이 목록+도메인이 생성할 에이전트·스킬 설계의 근거가 된다 — 구체적일수록 정확해진다.
# ─────────────────────────────────────────────
...
### 실행 순서 (steps/ 폴더 = 단계). step 전용 지침은 각 step 파일에 내장, 횡단 kb만 로드맵대로 로드.
0. `steps/0_setup.md`     → 위 입력으로 스택 확정. 비운 버전은 step0 내 선정 규칙으로 검색·선정 후 표로 승인.
1. `steps/1_analyze.md`   → greenfield/existing 판정 + (existing이면) 레포 탐색·보고. 파일 생성 금지.
2. `steps/2_sdd.md`       → `docs/` SDD 두뇌 생성: prd.md(+## MVP 제외)·architecture.md·adr/.
3. `steps/3_agents.md`    → 도구+도메인+discovery 근거로 에이전트 목록 제안·승인 후 `.claude/agents/*` 생성.
4. `steps/4_skills.md`    → 에이전트별 반복 워크플로를 스킬로 제안·승인 후 `.claude/skills/*` + 오케스트레이터 생성.
5. `steps/5_constitution.md` → 루트 `CLAUDE.md`(헌법) + `.claude/settings.json` + `.claude/hooks/*` 생성.
6. `steps/6_engine.md`    → `scripts/{execute.sh,phase.json}` + `.claude/commands/{harness,review}.md` 생성.
7. `steps/7_validate.md`  → 구조·훅·엔진·트리거·토큰 게이트 검증.
공통: 안전·TDD 규칙은 `kb/safety-rules.md`를 산출물 헌법 `CLAUDE.md`(# CRITICAL)와 훅에 반영.
공통: 각 step 종료 시 `kb/docs-rules.md`(result_report)대로 `docs/result_report/`에 짧은 리포트.
공통: "작업지시서 참고" 지시가 있으면 작업 전 `kb/docs-rules.md`(work_orders)대로 `docs/work_orders/`를 먼저 읽어라.
...
- 도구+도메인+discovery를 근거로 에이전트·스킬을 **템플릿(작성 지침 내장) 기반으로 설계·생성**. 작업 시엔 필요한 것만 활성화.
- 산출물(`docs/` SDD + `scripts/` 엔진 + `CLAUDE.md` 헌법 + `.claude/`의 commands·agents·skills·hooks)이
