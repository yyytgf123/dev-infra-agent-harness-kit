#!/usr/bin/env bash
# 산출물: scripts/execute.sh (step6). 자율 phase 실행 엔진 — bash+jq만.
# 동작: phase.json의 phases[]를 의존성 순으로 하나씩 headless `claude -p`로 실행하고,
#       각 phase 후 test_cmd를 결정적으로 돌려 통과해야 다음으로 넘어간다.
# 안전: 사람 승인 게이트(approved) + 서킷브레이커(연속 N회 실패 시 정지) + phase별 wall-clock 타임아웃.
# phase.json은 이 스크립트만 쓴다(모델은 읽기 전용). 사용: `bash scripts/execute.sh`
set -uo pipefail
...
record_error() {  # $1=phase id, $2=message
  local id="$1" msg="$2"
...
# --- 게이트 0: 전제조건 (전제가 깨지면 뒤 게이트가 조용히 무력화되므로 여기서 명시적으로 실패) ---
git -C "$ROOT" rev-parse --is-inside-work-tree >/dev/null 2>&1 || {
  echo "git 저장소가 아닙니다. 커밋 분리·민감파일 게이트(safety.sh)가 무력화됩니다 → git init + 초기 커밋 후 재실행."; exit 4; }
command -v claude >/dev/null || { echo "claude CLI 미설치 — 자율 실행 불가."; exit 4; }
[ "$(jq -r '.phases | length' "$STATE")" -gt 0 ] || {
  echo "phases[]가 비어 있습니다. 먼저 harness.md(단계 분할)를 실행하세요."; exit 4; }
# env_checks: step6이 확정 스택으로 채움. 필드 없으면 skip(구버전 호환).
if [ "$(jq -r '(.env_checks // []) | length' "$STATE")" -gt 0 ]; then
  jq -c '.env_checks[]' "$STATE" | while read -r chk; do
    C=$(printf '%s' "$chk" | jq -r '.cmd'); E=$(printf '%s' "$chk" | jq -r '.expect')
    OUT=$(eval "$C" 2>&1 | head -3)
    printf '%s' "$OUT" | grep -q "$E" || {
      echo "환경 불일치: [$C] 결과에 '$E' 없음 → 실제: $(printf '%s' "$OUT" | head -1)"; echo "확정 스택과 실행 환경이 다르면 게이트(spotless 등)가 깨진 채 돕니다. 환경을 맞추거나 step0 버전 표를 재승인하세요."; exit 4; }
  done || exit 4
fi
# 크래시 고착 복구: 이전 실행이 죽어 running/testing에 멈춘 phase를 pending으로 리셋(재개 가능)
jqi '(.phases[] | select(.status=="running" or .status=="testing") | .status) = "pending" | .current=null'
...
while :; do
  ...
  if [ -n "$MAXRUN" ] && [ "$ran" -ge "$MAXRUN" ]; then
    echo "⏸ max_phases_per_run($MAXRUN) 도달 — 이번 실행 종료. 다시 실행하면 이어서 진행."; exit 0
  fi

  # 다음 실행 가능한 phase: pending이고 depends_on이 전부 done
  ...
  if [ -z "$PID" ]; then
    ...
    echo "⚠️ 실행 가능한 pending phase 없음(미완 $REMAIN개 — failed/blocked 또는 순환 의존). phase.json 점검 필요."; exit 1
  fi
  ...
  # 격리 headless 세션(컨텍스트 누적 방지). 상태는 phase.json+디스크로만 복구.
  PROMPT=$(cat <<EOF
...
EOF
  ...
  if [ -n "$TCMD" ]; then
    ...
    # test_cmd 없으면(문서/설계 phase 등) 세션 성공만으로 done 처리
    jqi --arg id "$PID" --arg ts "$(date -u +%FT%TZ)" \
