...
description: >
  기존 코드에 대한 통합/e2e 테스트를 신규 추가하거나 커버리지를 확장한다(Testcontainers PostgreSQL 활용).
...
## 언제 쓰나 / 안 쓰나
- 트리거: 검증되지 않은 플로우(주문 생성→결제 처리 등)에 통합 테스트 추가, 경계/에러 케이스 e2e 보강
...
- 검증: `./gradlew test --tests '*OrderFlowTest'` (현 상태 확인)

### Step 2 — 테스트 작성
실제 PostgreSQL(Testcontainers)을 띄워 시나리오를 재현하는 `@SpringBootTest` 통합 테스트 작성. 의미 있는 단언 포함(상태 전이·DB 영속화 확인).
- 검증: `./gradlew compileTestJava`

### Step 3 — 실행/안정화
- 검증: `./gradlew test --tests '*OrderPaymentIntegrationTest'`

### Step 4 — 완료 검증 (필수)
- [ ] `./gradlew test` 통과(플레이키 없음)
- [ ] 범위 밖 파일 수정 없음(프로덕션 코드 무변경)
- [ ] 추가한 케이스가 실제 경계/에러를 검증
- [ ] (해당 시) `_workspace/`에 커버된 시나리오 기록 → qa 교차검증용
