...
## 2. 스펙 대비 검증 (SDD)
- `docs/prd.md` 수용 기준을 체크리스트로 대조 — 각 기준이 통과 테스트/산출물 증거와 매핑되는지.
- 미충족 = 미완료로 보고. `## MVP 제외` 항목이 구현돼 있으면 범위 초과로 보고.

## 2. 경계면 교차 QA (경계면 7패턴 — steps/3 QA 기준)
...
- 트리거 오발 → 스킬 description 강화. 반복 워크플로 → 새 스킬. 승격은 헌법·커맨드 기준

## 5. 리포트
- `docs/result_report/YYYYMMDD_HHMMSS.md`에 검증 결과 + 적용한 강화를 짧게(`헌법 Docs 섹션` result_report 형식).
