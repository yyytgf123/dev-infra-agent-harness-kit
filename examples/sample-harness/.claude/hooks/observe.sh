...
# $1 = pre|post. 입력은 stdin JSON으로 전달됨(.tool_name / .tool_input.command|.file_path). 명세 주석은 이 파일이 단일 소스(개념: kb/evolution.md).
PHASE="$1"
...
case "$TARGET" in
  *.env|*-prod.*|*tfstate*|*kubeconfig*|*.pem) RECORD="$(date -u +%FT%TZ)|$PHASE|$TOOL|<sensitive-redacted>";;
esac
