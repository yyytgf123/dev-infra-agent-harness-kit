...
# 2) 새 구조 산출물 존재
for p in CLAUDE.md .claude/commands/harness.md .claude/commands/review.md docs/prd.md docs/architecture.md \
...
{ jq -e . "$SAMPLE/.claude/settings.json" >/dev/null 2>&1 || python3 -c "import json,sys;json.load(open(sys.argv[1]))" "$SAMPLE/.claude/settings.json" 2>/dev/null; } && note "settings.json valid" "OK" || { note "settings.json valid" "FAIL"; fail=1; }
{ jq -e . "$SAMPLE/scripts/phase.json" >/dev/null 2>&1 || python3 -c "import json,sys;json.load(open(sys.argv[1]))" "$SAMPLE/scripts/phase.json" 2>/dev/null; } && note "phase.json valid" "OK" || { note "phase.json valid" "FAIL"; fail=1; }
...
# 6) v3.2 규격: 플랫 스킬 금지 / 비표준 frontmatter 금지 / JSON 파이썬 검증(jq 없는 환경 폴백)
if find "$SAMPLE/.claude/skills" -maxdepth 1 -name "*.md" 2>/dev/null | grep -q .; then
  note "no flat skill files" "FAIL (need <name>/SKILL.md)"; fail=1
else note "no flat skill files" "OK"; fi
if grep -l "^scope:" "$SAMPLE"/.claude/agents/*.md >/dev/null 2>&1 || grep -l "^subagent_type:" "$SAMPLE"/.claude/agents/*.md >/dev/null 2>&1; then
  note "no non-standard agent frontmatter" "FAIL"; fail=1
else note "no non-standard agent frontmatter" "OK"; fi
