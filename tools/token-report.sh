...
build_report() {
  ...
  # steps, kb, templates (빌드 시 단계별로 로드됨)
  for area in steps kb templates; do
...
runtime_report() {
  ...
  local consti=""
  for c in CLAUDE.md claude.md; do [ -f "$proj/$c" ] && consti="$proj/$c" && break; done
...
dup_report() {
  ...
  dup_one() { # label  pattern
    local n
    n=$(grep -rlE "$2" "$ROOT/steps" "$ROOT/kb" "$ROOT/templates" 2>/dev/null | wc -l | tr -d ' ')
    printf '%-34s %3s files\n' "$1" "$n"
  }
