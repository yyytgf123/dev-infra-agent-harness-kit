...
`steps/`(지침)·`kb/`(지식)와 분리해 여기 모았다. AGENT/SKILL 템플릿에는 작성 지침 주석이 내장돼 있다. 사용자가 채우는 파일이 아니다.
...
| `AGENT.md.tmpl` | 3 | `.claude/agents/{name}.md` (step3가 지침 주석대로 채움) |
| `SKILL.md.tmpl` | 4 | `.claude/skills/{name}/SKILL.md` (step4가 지침 주석대로 채움) |
...
## 흐름
```
steps/(지침 내장) · kb/(횡단 지식)  ──읽고──▶  templates/**/*.tmpl  ──떠서──▶  프로젝트/{CLAUDE.md,docs/,scripts/,.claude/}
```
...
- **스킬은 필요한 만큼 만들되 상시 비용은 lean** — 스킬 description만 상시, 본문은 트리거 시(Progressive Disclosure).
- **skill 본문 ≤500줄**, 도구별 상세는 `references/`로 내려 트리거 시 관련 파일만 로드.

## 새 산출물 유형 폴백
대응 템플릿이 없는 산출물이 필요하면: 가장 유사한 기존 tmpl 형식을 준용해 생성하고,
반복 사용되면 templates/에 tmpl로 역수입한다(사람 승인 후).
